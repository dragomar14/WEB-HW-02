...
from outputs import NotebookOutput
import pickle
import re
...
class NoteBook(UserDict):
    ...
    def search_note(self, symb):
        ...
        for rec in self.data:
            if symb.lower() in str(self.data[rec].note).lower():
                result.append([rec, ", ".join(
                    p.value for p in self.data[rec].tags), self.data[rec].note.value])
        if result:
    ...
    def show_all(self):
        ...
        for index, wr in self.data.items():
            result.append(
                [index, ", ".join(p.value for p in wr.tags), wr.note.value])
        return self.output.create_table(result)
